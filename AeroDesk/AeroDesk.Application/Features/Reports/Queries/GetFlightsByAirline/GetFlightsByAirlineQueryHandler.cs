...
{
    public class GetFlightsByAirlineQueryHandler
        : IRequestHandler<GetFlightsByAirlineQuery, List<FlightReportDto>>
    {
        private readonly IApplicationDbContext _context;

        public GetFlightsByAirlineQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }

        public async Task<List<FlightReportDto>> Handle(
            GetFlightsByAirlineQuery request,
            CancellationToken cancellationToken)
        {
            return await _context.Flights
                .AsNoTracking()
                .Where(f => f.AirlineId == request.AirlineId)
